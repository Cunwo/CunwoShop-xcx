...
import {
  GoodsDetailModel
} from '../goods-details/goods-details-model.js'
...
Page({
  ...
  toPayOrder: function() {
    ...
    for (let i = 0; i < shopList.length; i++) {
      ...
      //检查价格和库存
      goodsModel.getProductPriceInfo(carShopBean.goodsId, carShopBean.propertyChildIds, (res) => {
        console.log("检查价格和库存")
        console.log(res)
        if(res.status ==  0 ){
          //数据绑定
          doneNumber++;
          console.log("库存是:" + res.data.stocks)
          if (res.data.stocks < carShopBean.number) {
            wx.showModal({
              title: '提示',
          ...
          console.log("库存是:" + res.data.price)
          ...
          wx.showModal({
            title: '提示',
            content: "【"+carShopBean.name +"】" + res.msg + ","+" 请重新选择属性购买",
            showCancel: false
          })
          isFail = true;
          wx.hideLoading();
  ...
  navigateToPayOrder: function() {
    wx.hideLoading();
    wx.navigateTo({
      url: "/pages/to-pay-order/index?orderType=''"
    })
  }
